    ...
    public class Note2Controller : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            using (var db = new AspnetNoteDbContext())
            {
                ...
                ViewBag.notes = db.Notes2.ToList();
        ...
        public IActionResult AddAnswer(int noteNo)
        {
            ...
            using (var db = new AspnetNoteDbContext())
            {
                var note = db.Notes2.FirstOrDefault(n => n.NoteNo.Equals(noteNo));
                note.ReadNum++;
                db.SaveChanges();
                return View(note);
            }
        ...
        [HttpPost]
        public IActionResult AddAnswer(Note2 model)
        {
            if (HttpContext.Session.GetInt32("USER_LOGIN_KEY") == null)
            {
                //로그인이 안된 상태
            ...
            using (var db = new AspnetNoteDbContext())
            {
                var note = db.Notes2.FirstOrDefault(n => n.NoteNo.Equals(model.NoteNo));
                note.NoteAnswer = model.NoteAnswer;
        ...
        [HttpPost]
        public IActionResult Add(Note2 model)
        {
            if (HttpContext.Session.GetInt32("USER_LOGIN_KEY") == null)
            {
                //로그인이 안된 상태
