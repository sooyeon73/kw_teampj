...
namespace AspnetCoreStudy.Controllers
{
    ...
    public class Note1Controller : Controller
    {
        public int num;
        /// <summary>
        /// 게시판 리스트
        ...
        public IActionResult Index()
        {
            ...
            using (var db = new AspnetNoteDbContext())
            {
                ...
                ViewBag.notes = db.Notes1.ToList();
        ...
        public IActionResult Detail(int noteNo)
        {
            ...
            using (var db = new AspnetNoteDbContext())
            {
                var note = db.Notes1.FirstOrDefault(n => n.NoteNo.Equals(noteNo));
        ...
        [HttpPost]
        public IActionResult Detail(Note1 model)
        {
            if (HttpContext.Session.GetInt32("USER_LOGIN_KEY") == null)
            {
                //로그인이 안된 상태
            ...
            using (var db = new AspnetNoteDbContext())
            {
                ...
                var no = db.Notes1.FirstOrDefault(n => n.NoteNo.Equals(model.NoteNo));

                var a = new Comment();
                a.CommentContents = model.Comment1;
                a.NoteID = model.NoteNo;
                db.Comments.Add(a);
            ...
            return Detail(model.NoteNo);
        }
        [HttpPost]
        public IActionResult Read(Note1 model)
        {
            if (HttpContext.Session.GetInt32("USER_LOGIN_KEY") == null)
            {
                //로그인이 안된 상태
        ...
        [HttpPost]
        public IActionResult Add(Note1 model)
        {
            if (HttpContext.Session.GetInt32("USER_LOGIN_KEY") == null)
            {
                //로그인이 안된 상태
        ...
        ////
        [HttpPost]
        public IActionResult Edit(Note1 model)
        {
            if (HttpContext.Session.GetInt32("USER_LOGIN_KEY") == null)
            {
                //로그인이 안된 상태
        ...
        public IActionResult Delete(int noteNo)
        {
            ...
            if (ModelState.IsValid)
            {
                using (var db = new AspnetNoteDbContext())
                {
                    Note1 note = db.Notes1.Find(noteNo);
                    /*   if (note.UserNo != int.Parse(HttpContext.Session.GetInt32("USER_LOGIN_KEY").ToString()))
                       {
                           return Redirect("Index");
                       }*/

                    db.Notes1.Remove(note);
                    db.SaveChanges();
                    return Redirect("Index");
                }
                ModelState.AddModelError(string.Empty, "에러.");
        ...
        public IActionResult Add_(IFormFile files)
        {
            ...
            using (var db = new AspnetNoteDbContext())
            {
                ...
                if (files != null)
                {
                    ...
                    {
                        n = db.Notes1.Last().NoteNo + 1; // 새로운 노트 번호
                    }

                    Files ob = null;
                    foreach (Files f in db.Files)
                    {
                        if (f.NoteNo == n && f.BoardNo == 1) // board num 1
                        {
                            ob = f;
                        }
                    }
